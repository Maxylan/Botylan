...
function intervalASwitch(message) {
...
function stopIntervalAS(message) {
    clearInterval(activitySwitcher);
    activityTimerStatus = 1;
    cleanup(message,
        ("Activity Cooldown/Timer now **stopped**"));
}
function startIntervalAS(message) {
    activitySwitcher = setInterval(() => { switchActivity(true, false, 0, false); }, activitySwitchInterval);
    activityTimerStatus = 0;
    cleanup(message,
        ("Activity Cooldown/Timer now **resumed**"));
}
    ...
    function filterPassed() {
        try {
            ...
            function saveRandMessage() {
                let coinToss = Math.floor((Math.random() * 4) + 1);
                if (coinToss === 2) {
            ...
            function saveQuoteMessage() {
                runPassiveCommand("quotemessage");
            }
        ...
        try {
            if (permissions >= client.commands.get(command).perm) {
                if (client.commands.get(command).perm === 1) {
                    ...
                    client.commands.get(command).execute(message, args, fs, t1Commands, t2Commands, t3Commands);
                }
                else if (client.commands.get(command).perm === 2) {
                    modCommandFired(command, message);
                    client.commands.get(command).execute(message, client, args, fs, convosFile, cleanup, filter, switchActivity, activityPresets, aPresetsRand, intervalASwitch, stopIntervalAS, startIntervalAS, activityTimerStatus);
                }
